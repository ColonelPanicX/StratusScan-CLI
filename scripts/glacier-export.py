...
def main():
    ...
    all_available_regions = utils.get_all_aws_regions(service_code='glacier')
    default_regions = utils.get_partition_regions(partition, all_regions=False)

    # Process selection
    if selection_int == 1:
        regions = default_regions
        utils.log_info(f"Selected default regions: {len(regions)} regions")
        ...
        while True:
            try:
                ...
                if 0 <= region_idx < len(all_available_regions):
                    ...
                    utils.log_info(f"Selected region: {selected_region}")
                    break
                else:
