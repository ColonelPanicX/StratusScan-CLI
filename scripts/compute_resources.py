...
Orchestrates all compute resource exporters as subprocesses and archives
...
# ---------------------------------------------------------------------------
COMPUTE_SCRIPTS: List[Tuple[str, str]] = [
    ("EC2",                       "ec2_export.py"),
    ("EKS",                       "eks_export.py"),
    ("ECS",                       "ecs_export.py"),
...
def prompt_script_selection(
    ...
    while True:
        print("\nSELECT COMPUTE RESOURCES TO EXPORT")
        print("=" * 64)
        print("   0. All  (export all compute resources)")
...
def create_zip_archive(
    ...
    zip_name = f"{account_name}-compute-resources-all-export-{date}.zip"
    ...
    if zip_path.exists():
        ...
        while True:
            candidate = output_dir / (
                f"{account_name}-compute-resources-all-export-{date}-v{v}.zip"
            )
...
def print_summary(
    ...
    print(f"\n{'=' * 70}")
    print("COMPUTE RESOURCES EXPORT — SUMMARY")
    print(f"{'=' * 70}")
...
def main() -> None:
    account_id, account_name = utils.print_script_banner(
        "COMPUTE RESOURCES ALL-IN-ONE EXPORT"
    )
    ...
    while True:
        ...
        if step == 1:
            result = utils.prompt_region_selection("Compute Resources")
            if result == 'back':
                sys.exit(10)
            if result == 'exit':
                sys.exit(11)
        ...
        # ── Step 2: Script selection ──────────────────────────────────────
            ...
            result = prompt_script_selection(COMPUTE_SCRIPTS)
            if result == 'back':
                step = 1
                continue
            if result == 'exit':
        ...
        # ── Step 3: Confirmation ──────────────────────────────────────────
            ...
            msg = (
                ...
                f"compute resource(s):\n"
                f"{script_lines}\n\n"
                f"  Regions : {region_str}\n"
                f"  Output  : {output_dir / (account_name + '-compute-resources-all-export-<date>.zip')}"
            )
